...
const initializeGame = (sio, socket) => {
  ...
  gameSocket.on('chatMsg', chatMsg)
  gameSocket.on('resign', resign)
...
function chatMsg(msgData) {
  io.to(msgData.gameId).emit("chatMsgReceive", { msg: msgData.msg, name: msgData.name })
}

function resign(gameId, color) {
  io.to(gameId).emit("player has resigned", color)
}
...
function newMove(move) {
  ...
  io.to(gameId).emit('moveLog', move);
}
